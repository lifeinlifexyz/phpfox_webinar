...
class Webinar_Service_Category_Category extends Phpfox_Service
{
	private $_sOutput = '';
	...
	{		
		$sCacheId = $this->cache()->set('webinar_category_browse' . ($iCategoryId === null ? '' : '_' . $iCategoryId));
		...
		{					
			...
			{
				$aCategories[$iKey]['url'] = Phpfox::permalink('webinar.category', $aCategory['category_id'], $aCategory['name']);
				...
				//if ($sCategory === null)
				{
					...
					{
						$aCategories[$iKey]['sub'][$iSubKey]['url'] = Phpfox::permalink('webinar.category', $aSubCategory['category_id'], $aSubCategory['name']);
					}
				}
			}
	...
	public function get()
	{
		$sCacheId = $this->cache()->set('webinar_category_display_' . $this->_sDisplay . '_' . Phpfox::getLib('locale')->getLangId());
	...
	{		
		$sCacheId = $this->cache()->set('webinar_parent_breadcrumb_' . md5($sCategory));
	...
	public function getCategoriesById($iId = null, &$aCategories = null)
	{
		...
		if ($aCategories === null)
		{
			$aCategories = $this->database()->select('pc.parent_id, pc.category_id, pc.name')
				->from(Phpfox::getT('webinar_category_data'), 'pcd')
				->join($this->_sTable, 'pc', 'pc.category_id = pcd.category_id')
				->where('pcd.webinar_id = ' . (int) $iId)
		...
		{			
			...
			{				
				$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategory['name']), Phpfox::permalink('webinar.category', $aCategory['category_id'], $aCategory['name']));
			}
		}		
		else 
		{			
			$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategories[0]['name']), Phpfox::permalink('webinar.category', $aCategories[0]['category_id'], $aCategories[0]['name']));
		}
	...
	public function getCategoryIds($iId)
	{
		$aCategories = $this->database()->select('category_id')
			->from(Phpfox::getT('webinar_category_data'))
	...
	{
		$sCacheId = $this->cache()->set('webinar_category_childern_' . $sCategory);
	...
	{
		$sCacheId = $this->cache()->set('webinar_category_parent_' . $sCategory);
	...
	public function __call($sMethod, $aArguments)
	{
		 ...
		 */
		if ($sPlugin = Phpfox_Plugin::get('webinar.service_category_category__call'))
		{
			return eval($sPlugin);
		}
	...
	private function _get($iParentId, $iActive = null)
	{
		...
		if (count($aCategories))
		{
			...
			foreach ($aCategories as $iKey => $aCategory)
			{
				...
				elseif ($this->_sDisplay == 'admincp')
				{
					$sOutput .= '<li><img src="' . Phpfox::getLib('template')->getStyle('image', 'misc/move.png') . '" alt="" /> <input type="hidden" name="order[' . $aCategory['category_id'] . ']" value="' . $aCategory['ordering'] . '" class="js_mp_order" /><a href="#?id=' . $aCategory['category_id'] . '" class="js_drop_down">' . Phpfox::getLib('locale')->convert($aCategory['name']) . '</a>' . $this->_get($aCategory['category_id'], $iActive) . '</li>' . "\n";
				}
				else 
				{				
	...
	private function _getParentsUrl($iParentId, $bPassName = false)
	{
		...
		if ($iCnt === 1)
		{
			...
			$sCacheId = $this->cache()->set('webinar_category_url' . ($bPassName ? '_name' : '') . '_' . $iParentId);
